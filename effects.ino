void flipTick() {
  if (FLIP_EFFECT == 0) {
    ...
    if (flipTimer.isReady()) {
      ...
      if (flipCounter == FULL_LAMP) {  // если ни одну из 4 цифр менять не нужно
        // выходим из цикла изменения
        flipInit = false;
        newTimeFlag = false;
      }
  ...
  // --- train --- //
    ...
    if (flipTimer.isReady()) {
      if (trainLeaving) {
        for (byte i = FULL_LAMP - 1; i > currentLamp; i--) {
          indiDigits[i] = indiDigits[i - 1];
        }
        anodeStates[currentLamp] = 0;
        currentLamp++;
        if (currentLamp >= FULL_LAMP) {
        ...
        indiDigits[0] = newTime[FULL_LAMP - 1 - currentLamp];
